...
with open('gen_files/exp_sim_anneal_suc'+str(n)+'.prism', 'w') as f:
    f.write('dtmc\n\n')
    ...
    f.write('\t[] (valid=0) & (total_atk > 0) & (success_swap_counter<N) -> ')
    for j in range(1, n):
    ...
    f.write('formula overall_attack_num = ')
    for j in range(1, n):
        for i in range(j+1, n+1):
            f.write('((q'+str(j)+'x-q'+str(i)+'x) = (q'+str(j)+'y-q'+str(i)+'y) | (q'+str(j)+'x-q'+str(i)+'x) = -(q'+str(j)+'y-q'+str(i)+'y))?1:0 + ')
            if j==n-1 and i==n:
                f.write('(q'+str(j)+'x = q'+str(i)+'x)?1:0;\n\n')
            else:
                f.write('(q'+str(j)+'x = q'+str(i)+'x)?1:0 + ')
